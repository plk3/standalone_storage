...
function filterTags() {
  ...
  // 1. Filter
  let filtered = availableTags.filter((t) =>
    t.name.toLowerCase().includes(query)
  );

  // 2. Sort
  const sortType = document.querySelector(
    'input[name="sortTags"]:checked'
  ).value;
  filtered.sort((a, b) => {
    if (sortType === "count") {
...
async function loadFiles(reset = false) {
  ...
  try {
    const res = await fetch(
      `${API_URL}/files?q=${encodeURIComponent(
        query
    ...
    if (newFiles.length > 0) {
      ...
      if (reset) {
        document.getElementById("fileGrid").innerHTML =
          '<p style="text-align:center;grid-column:1/-1;">No files found.</p>';
      }
    }
  } catch (err) {
    console.error(err);
  ...
  files.forEach((file) => {
    ...
    card.innerHTML = `
      ...
      file.filename
            ...
            <div class="card-body editing" id="edit-${
              file.id
            }" style="display:none;">

                <input type="text" id="edit-tags-${file.id}" value="${(
      file.tags || []
    ).join(", ")}" placeholder="Tags">
            </div>

            <div class="card-actions">
                <button class="btn-secondary btn-sm" onclick="toggleEdit('${
                  file.id
                }')" id="btn-edit-${file.id}">Edit</button>
                <button class="btn-primary btn-sm" onclick="saveEdit('${
                  file.id
                }')" id="btn-save-${
      file.id
    }" style="display:none;">Save</button>
                <button class="btn-danger btn-sm" onclick="deleteFile('${
                  file.id
                }')">Delete</button>
                <a href="${
                  file.url
                }" class="btn-primary btn-sm" style="text-decoration:none;" download>⬇</a>
            </div>
        `;
...
window.toggleEdit = (id) => {
  ...
  if (edit.style.display === "none") {
    ...
    // Focus input
    setTimeout(() => document.getElementById(`edit-tags-${id}`).focus(), 0);
  } else {
    // Cancel Edit Mode
...
async function showImage(index) {
  ...
  try {
    const res = await fetch(
      `${API_URL}/files/${file.id}/download?preview=true`
    );
...
window.uploadRestore = async () => {
  ...
  if (
    !confirm(
      "Restoring will overwrite existing files with the same name. Continue?"
    )
  ) {
